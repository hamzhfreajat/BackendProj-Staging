...
with open('final_queries.txt', 'r', encoding='utf-8') as f:
    queries = [line.strip() for line in f if line.strip() and not line.startswith('🏡') and not line.startswith('💰') and not line.startswith('📍') and not line.startswith('🛏') and not line.startswith('🛋') and not line.startswith('🚗') and not line.startswith('🏢') and not line.startswith('📐') and not line.startswith('👨') and not line.startswith('🔥') and not line.startswith('هذه أقرب')]
...
xls_path = 'C:/Users/hfraijat/.gemini/antigravity/brain/696fb396-0dce-4ae5-a16f-7a3bb2cd2761/full_test_results.xls'
